...
const contract = new web3.eth.Contract(con.abi, con.address);
web3.eth
  .getAccounts()
  .then(res => {})
  .catch(err => {
...
exports.transferFrom = async function(from, to, value) {
  if (isAddress(from) && isAddress(to) && from !== to) {
    ...
    } else console.error("Error: ", "Invalid amount entered");
  } else {
...
exports.bid = async function(week, amount) {
  ...
  } else console.error("Error: ", "Invalid args (week>=1, amount>=1)");
};
...
exports.balanceOf = async function(address) {
  ...
  } else console.error("Error: Invalid address");
};
...
exports.allowance = async function(owner, spender) {
  ...
  } else console.error("Error: Invalid addresses");
};
...
exports.approve = async function(spender, value) {
  ...
  } else console.error("Error: Invalid address or value");
};
...
exports.fallback = async function(value) {
  ...
  } else console.error("Error: Invalid value entered (value> 0)");
};
...
exports.getCurrentBid = async function(week) {
  return new Promise((resolve, reject) => {
    let result;
    contract.methods
      .currentBid(week, con.fixed_call.from)
      .call(con.fixed_call)
      .then(res => {
        result = res;
        resolve(result);
